...
//        die;
    ...
    public function notify($postData, $sign, $get)
    {
        ...
        if ($pre == 'GZ') {//大为那边的
            $a = file_get_contents("http://dey6kxqf.game2019.net/?d=Game&c=GamePayMent&a=updateOrderStatus&status=".$data['status']."&thirdorderid=".$data['order_number']."&amount=".$data['amount']);
            exit;
        }
